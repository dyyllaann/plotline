    ...
    for y in range(1, H + 1):
        for x in range(1, W + 1):
            ...
            if angle == 0 or angle == 180:
                neighbors = [padded_g[y, x - 1], padded_g[y, x + 1]]
            elif angle == 45 or angle == 225:
                neighbors = [padded_g[y - 1, x - 1], padded_g[y + 1, x + 1]]
            elif angle == 90 or angle == 270:
                neighbors = [padded_g[y - 1, x], padded_g[y + 1, x]]
            else:
                neighbors = [padded_g[y - 1, x + 1], padded_g[y + 1, x - 1]]

            if padded_g[y, x] > max(neighbors):
                out[y - 1, x - 1] = padded_g[y, x]
...
def link_edges(strong_edges, weak_edges):
    ...
    queue = list(zip(*np.nonzero(strong_edges)))
...
def hough_transform(img):
    ...
    for x, y in zip(xs, ys):
      ...
      for theta_idx, rho_idx in enumerate(rho_indices):
